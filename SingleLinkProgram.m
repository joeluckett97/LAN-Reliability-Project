%% Function SingleLinkProgram()
%
% This network has one link
%
...
function SingleLinkProgram()
    ...
    for kCounter = 1:length(kValues) % runs through all iterations for the amount of packets
        ...
        for pCounter = 1:length(pValues)
            p = pValues(pCounter);
            simulatedResults(kCounter, pCounter) = runSingleLinkSim(k, p, n); % simulates amount of transmissions
